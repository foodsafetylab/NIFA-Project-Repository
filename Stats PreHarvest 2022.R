...
####################################################01/16/23
#Test for variances - Prior performing F-tests, we split the data into the groups we desire to compare:
#Swabs v Tissues
##Prehydrated Swabs v Dry Swabs
##Prehydrated swabs - Hand v MSD swabs 
##DRy swabs- Hand v MSD swabs 
...
var_APC<-Everything_wet%>%filter(Test== "APC")
var_colif<-Everything_wet%>%filter(Test== "Coliforms")

APCvariance<-var_APC%>% #Aggregative Swab= 0.8790569; Produce Sample=0.1278418
   group_by(Sample.Type)%>%
   summarise(var=var(Best..Estimate))

Colivariance<-var_colif%>% #Aggregative Swab= 0.88091162; Produce Sample=0.6376682
  group_by(Sample.Type)%>%
  summarise(var=var(Best..Estimate))


#variance test
var.test(Best..Estimate~Sample.Type, data=var_APC) #p=6.705e-05
var.test(Best..Estimate~Sample.Type, data=var_colif) #p=0.4682
