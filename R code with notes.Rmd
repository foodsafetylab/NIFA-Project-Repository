...
Sample type and inoculation status were added as columns before performing the ANOVA.
```{r setup, eval=FALSE}
rarefied_alpha_diveristy_store_remove$Type<- meta_file_store$Type
rarefied_alpha_diveristy_store_remove$Inoculation<- meta_file_store$Inoculation

AlphaDiv<- rarefied_alpha_diveristy_store_remove
```
...
This was done for visualization purposes. Originally, when you look at the taxonomic rank of species you will just see the species name and not the corresponding genus name. This code chunk makes it so the species ranking will now appear as the genus and species name. For example, instead of just "coli", the species name will now appear as "Escherichia coli". 
```{r setup, eval=FALSE}
...
#Subsetting sample types - JQ edit: Use the grouping argument in the "toptaxa" function (line 237 - beyond)
...
Each taxonomic rank was plotted separately. Colors were specified to be able to follow taxa that belonged to different phyla and classes throughout the plots at lower taxonomy rankings. For example, all taxa in the Gammaproteobacteria class are shades of blue throughout the various plots. The plots were then stacked on top of eachother to form one cohesive figure.
```{r}
...
#Expressing the abundance counts as relative abundance
The same figure as above, which contains the top 5 most abundant taxa at each taxonomical rank, was recreated. However, this time the abundance counts were first transformed to a relative abundance between 0 and 1.
```{r}
phyloRareTrans<- transform_sample_counts(phyloseq.rarefiedStorePrimeRemove, function(x) x/sum(x))

swabsTrans<- phyloseq::subset_samples(phyloRareTrans, Type== "Swab")
tissueTrans<- phyloseq::subset_samples(phyloRareTrans, Type== "Tissue")
storeTrans<- phyloseq::subset_samples(phyloRareTrans, Type== "Store Tissue")

tax.glom.phylum.trans.swabs<- tax_glom(swabsTrans, taxrank="Phylum")
tax.glom.phylum.trans.tissue<- tax_glom(tissueTrans, taxrank="Phylum")
tax.glom.phylum.trans.store<- tax_glom(storeTrans, taxrank="Phylum")

tax.glom.class.trans.swabs<- tax_glom(swabsTrans, taxrank="Class")
tax.glom.class.trans.tissue<- tax_glom(tissueTrans, taxrank="Class")
tax.glom.class.trans.store<- tax_glom(storeTrans, taxrank="Class")

tax.glom.order.trans.swabs<- tax_glom(swabsTrans, taxrank="Order")
tax.glom.order.trans.tissue<- tax_glom(tissueTrans, taxrank="Order")
tax.glom.order.trans.store<- tax_glom(storeTrans, taxrank="Order")

tax.glom.family.trans.swabs<- tax_glom(swabsTrans, taxrank="Family")
tax.glom.family.trans.tissue<- tax_glom(tissueTrans, taxrank="Family")
tax.glom.family.trans.store<- tax_glom(storeTrans, taxrank="Family")

tax.glom.genus.trans.swabs<- tax_glom(swabsTrans, taxrank="Genus")
tax.glom.genus.trans.tissue<- tax_glom(tissueTrans, taxrank="Genus")
tax.glom.genus.trans.store<- tax_glom(storeTrans, taxrank="Genus")

tax.glom.species.trans.swabs<- tax_glom(swabsTrans, taxrank="Species")
tax.glom.species.trans.tissue<- tax_glom(tissueTrans, taxrank="Species")
tax.glom.species.trans.store<- tax_glom(storeTrans, taxrank="Species")

top5phylumTransSwabs<- get_top_taxa(tax.glom.phylum.trans.swabs, n=5, relative=TRUE)
top5phylumTransTissue<- get_top_taxa(tax.glom.phylum.trans.tissue, n=5, relative=TRUE)
top5phylumTransStore<- get_top_taxa(tax.glom.phylum.trans.store, n=5, relative=TRUE)
top5phylumTrans<- merge_phyloseq(top5phylumTransSwabs, top5phylumTransTissue, top5phylumTransStore)

top5classTransSwabs<- get_top_taxa(tax.glom.class.trans.swabs, n=5, relative=TRUE)
top5classTransTissue<- get_top_taxa(tax.glom.class.trans.tissue, n=5, relative=TRUE)
top5classTransStore<- get_top_taxa(tax.glom.class.trans.store, n=5, relative=TRUE)
top5classTrans<- merge_phyloseq(top5classTransSwabs, top5classTransTissue, top5classTransStore)

top5orderTransSwabs<- get_top_taxa(tax.glom.order.trans.swabs, n=5, relative=TRUE)
top5orderTransTissue<- get_top_taxa(tax.glom.order.trans.tissue, n=5, relative=TRUE)
top5orderTransStore<- get_top_taxa(tax.glom.order.trans.store, n=5, relative=TRUE)
top5orderTrans<- merge_phyloseq(top5orderTransSwabs, top5orderTransTissue, top5orderTransStore)

top5familyTransSwabs<- get_top_taxa(tax.glom.family.trans.swabs, n=5, relative=TRUE)
top5familyTransTissue<- get_top_taxa(tax.glom.family.trans.tissue, n=5, relative=TRUE)
top5familyTransStore<- get_top_taxa(tax.glom.family.trans.store, n=5, relative=TRUE)
top5familyTrans<- merge_phyloseq(top5familyTransSwabs, top5familyTransTissue, top5familyTransStore)

top5genusTransSwabs<- get_top_taxa(tax.glom.genus.trans.swabs, n=5, relative=TRUE)
top5genusTransTissue<- get_top_taxa(tax.glom.genus.trans.tissue, n=5, relative=TRUE)
top5genusTransStore<- get_top_taxa(tax.glom.genus.trans.store, n=5, relative=TRUE)
top5genusTrans<- merge_phyloseq(top5genusTransSwabs, top5genusTransTissue, top5genusTransStore)

plot0Trans<- plot_bar(top5phylumTrans, x="Sample.Name", fill="Phylum", title="Most Abundant Phylas") + facet_wrap(~Type, scales="free_x") + scale_fill_manual(values = c("orchid3", "tomato3", "rosybrown1", "gold", "grey", "mediumturquoise", "lightcoral")) +
  theme(axis.text.x = element_text(size = 12), axis.text.y = element_text(size=12), axis.title = element_text(size=15), title=element_text(size=18), legend.text = element_text(size=15), strip.text=element_text(size=15)) + ylab("Relative Abundance")

plot1Trans<- plot_bar(top5classTrans, x="Sample.Name", fill="Class", title="Most Abundant Classes") + facet_wrap(~Type, scales="free_x")  + scale_fill_manual(values = c("orchid3", "mediumseagreen", "gold", "tomato3", "rosybrown1", "royalblue1", "grey", "lightcoral")) +
  theme(axis.text.x = element_text(size = 12), axis.text.y = element_text(size=12), axis.title = element_text(size=15), title=element_text(size=18), legend.text = element_text(size=15), strip.text=element_text(size=15))+ ylab("Relative Abundance")

plot2Trans<- plot_bar(top5orderTrans, x="Sample.Name", fill="Order", title="Most Abundant Orders") + facet_wrap(~Type, scales="free_x")  + scale_fill_manual(values = c("orange2", "olivedrab3", "rosybrown1", "lightskyblue", "gold", "tomato3", "orchid3", "grey", "royalblue1", "mediumseagreen")) +
  theme(axis.text.x = element_text(size = 12), axis.text.y = element_text(size=12), axis.title = element_text(size=15), title=element_text(size=18), legend.text = element_text(size=15), strip.text=element_text(size=15))+ ylab("Relative Abundance")

plot3Trans<- plot_bar(top5familyTrans, x="Sample.Name", fill="Family", title = "Most Abundant Families") + facet_wrap(~Type, scales="free_x")  + scale_fill_manual(values = c("orange2", "lightskyblue", "olivedrab3", "dodgerblue", "gold", "tomato3", "orchid3", "rosybrown1", "grey", "darkorange", "royalblue1", "mediumseagreen", "red4")) +
  theme(axis.text.x = element_text(size = 12), axis.text.y = element_text(size=12), axis.title = element_text(size=15), title=element_text(size=18), legend.text = element_text(size=15), strip.text=element_text(size=15))+ ylab("Relative Abundance")

plot4Trans<- plot_bar(top5genusTrans, x="Sample.Name", fill="Genus", title = "Most Abundant Genera") + facet_wrap(~Type, scales="free_x") + scale_fill_manual(values = c("mediumseagreen", "olivedrab3", "darkorange", "gold", "tomato3", "orange2", "grey", "dodgerblue", "red4", "olivedrab", "royalblue1", "blue")) +
  theme(axis.text.x = element_text(size = 12), axis.text.y = element_text(size=12), axis.title = element_text(size=15), title=element_text(size=18), legend.text = element_text(size=15), strip.text=element_text(size=15))+ ylab("Relative Abundance")

taxafigureTrans<- ggarrange(plot0Trans, plot1Trans, plot2Trans, plot3Trans, plot4Trans,
                            labels=c("a.","b.","c.","d.", "e."),
                            ncol=1, nrow=5, align= "hv", font.label = list(size=18)) 
taxafigureTrans1<- annotate_figure(taxafigureTrans, top= text_grob("Most Abundant Taxonomical Ranks", size = 25))
taxafigureTrans1
```


##Looking at rarer taxa
In previous code chunks, rarefied data was used. However, some of the more rare taxa were of interest in this study, such as the Enterobacteriaceae family. Since these taxa were expected to be rare, the non-rarefied read data was used. 
...
The following code creates a plot with the abundance counts, but these counts can be transformed to create a plot with the percent abundances.
...
The Pseudomonas genus was the most abundant genus in this study and if often found to be the most abundant genus in several studies of leafy green microbial communities. The species of Pseudomonas were investigated to see if known spoilage or plant pathogen species within this genus were present.
```{r setup, eval=FALSE}
