...
hist(log10(sample_sums(phyloseqAll)), main="Histogram: All Samples Read Counts", xlab="Log10 Total Reads", 
...
```
#Phyloseq by sample time point - no cloroplasts
```{r}
...
```
#Rarefication Preharvest w/ cloroplasts following dr S meeting
```{r}
...
#Rarefication of sample reads - to derivate accurate microbiota profiles from samples with low bacterial content 
```{r}
...
#Rarefication of sample reads - to derivate accurate microbiota profiles from samples with low bacterial content - following meeting with dr S
```{r}
phyloPrH_2KPrune32_rare<- rarefy_even_depth(twoK_prune_phylopreharvest, sample.size = 32, rngseed = 19)

phyloPrH_2KPrune63_rare<- rarefy_even_depth(twoK_prune_phylopreharvest, sample.size = 63, rngseed = 19)

phyloPrH_2KPrune125_rare<- rarefy_even_depth(twoK_prune_phylopreharvest, sample.size = 125, rngseed = 19)

phyloPrH_2KPrune250_rare<- rarefy_even_depth(twoK_prune_phylopreharvest, sample.size = 250, rngseed = 19)

phyloPrH_2KPrune500_rare<- rarefy_even_depth(twoK_prune_phylopreharvest, sample.size = 500, rngseed = 19)

phyloPrH_2KPrune1000_rare<- rarefy_even_depth(twoK_prune_phylopreharvest, sample.size = 1000, rngseed = 19)

phyloPrH_2KPrune1500_rare<- rarefy_even_depth(twoK_prune_phylopreharvest, sample.size = 1500, rngseed = 19)
```
##Rarefication and diversity tests following dr S meeeting
```{r}
...
#Alpha diveristy analysis- rarified above, preharvest only #prior dr S meeting
```{r}
#getting alpha diversity
...
```
#Rarefiying samples to 550 reads for preharvest only #Prior dr S meeting
```{r}
...
```
#Rarefiying samples to 63, 125, 250 and 500 reads for preharvest - no cloroplasts
```{r}
...
#Ploting alpha diversity for simpson - 63, 125 and 250
```{r}
...
```
#Alpha diveristy analysis- rare, preharvest all samples with >=63 reads
```{r}
#Running an ANOVA on the alpha diversities
